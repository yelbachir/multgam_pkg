...
- r-Largest extreme value distribution: `fmName="rgev"` implements `rGEV(mu, tau, xi)`, where `mu` is the location, `tau` is the log-scale and `xi` is the shape. The output variable `y` (say) in the argument `dat` of the function `mtgam` should be an nxr-matrix, where `n` is the number of blocks of data and `r` is the pre-specified number of largest extremal data per block, sorted in ascending order.
