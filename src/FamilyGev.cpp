...
double logLikGev(const Ref<const RowVectorXd>& yi, const Ref<const RowVectorXd>& thetai)
{
  ...
  if(std::abs(xi) <= std::pow(MachinePrec, 0.3)){
    ...
    double z1(z*xi);
    //			if( ((1+z1) < sqrTol) or (xi < -1) ){
    if((1+z1) < sqrTol){
      return -infiniteLL;

    } else {
      double L(std::log1p(z1)), xi1(1/xi);
      return -tau - L*(xi1 + 1) - std::exp(-L*xi1);
    };
  };
}
...
VectorXd Deriv1iGev(const Ref<const RowVectorXd>& yi, const Ref<const RowVectorXd>& thetai)
{
  ...
  */
  ...
  if(std::abs(xi) <= std::pow(MachinePrec, 0.3)){ // derivative components w.r.t thetai when xi==0
    ...
    //utility variables
    double L(std::log1p(b*xi)), xi1(1/xi);
    double e(std::expm1(-L*xi1));
    double exi1(e*xi1), f1(fexp(-tau,-1,L));

    Eigen::VectorXd out(3);
    out(0) = -f1*(e-xi);
    out(1) = a*out(0) -1;
    out(2) = a*f1*(exi1-1)-L*exi1*xi1;
...
VectorXd Deriv2iGev(const Ref<const RowVectorXd>& yi, const Ref<const RowVectorXd>& thetai)
{
  ...
  */
  ...
  if(std::abs(xi) <= std::pow(MachinePrec, 0.3)){ // derivative components w.r.t thetai when xi==0
    ...
    out(0) = -std::exp(-2*tau-b);
    out(1) = a*out(0)+e*et;
    out(2) = et*(b*(e-.5*b*eb)+1);
    out(3) = a*out(1);
    out(4) = a*out(2);
    out(5) = b2*(1-eb*.25*b2+2*e*b/3);
    ...
    //utility variables
    double xi1(1/xi), L(std::log1p(b*xi));
    double e(std::expm1(-L*xi1));
    double exi1(e*xi1), f1(fexp(-tau,-1,L)), f2(fexp(-2*tau,-2,L)), f0(fexp(0,-xi1,L));
    double f0xi1(f0*xi1), Lf0xi1(L*f0xi1), Lf0xi12(Lf0xi1*xi1), lmu(-f1*(e-xi));

    Eigen::VectorXd out(6);
    out(0) = (xi+1)*f2*(xi-f0);
    out(1) = a*out(0) -lmu;
    out(2) = a*f2*(e + f0xi1-xi) + f1*(1-Lf0xi12);
    out(3) = a*(out(0)*a - lmu);
    out(4) = a*out(2);
    out(5) = f2*(1-f0xi1*xi1-exi1)*a*a + (2*L*exi1 + 2*a*f1*(Lf0xi1 - e) - L*Lf0xi12)*xi1*xi1;
...
MatrixXd Deriv3iGev(const Ref<const RowVectorXd>& yi, const Ref<const RowVectorXd>& thetai)
{
  ...
  */
  ...
  if(std::abs(xi) <= std::pow(MachinePrec,0.3)){ // derivative components w.r.t thetai when xi==0
    ...
    Eigen::MatrixXd out(6,3);
    out(0,0) = -std::exp(-3*tau-b);
    out(1,0) = a*out(0,0)-2*lmumu;
    out(2,0) = std::exp(-2*tau)*(-e+beb*(2-.5*b));
    out(3,0) = out(0,0)*a*a -3*a*lmumu -e*et;
    out(4,0) = a*out(2,0)-et*(b*(e-.5*beb)+1);
    out(5,0) = b*et*(2*(b-1)+beb*(-2+5*b/3-.25*b2));
    ...
    out(3,2) = out(4,1);
    out(4,2) = out(5,1);
    out(5,2) = (2 - 1.5*e*b + (b2 -b3/8)*eb )*b3;
    ...
    Eigen::MatrixXd out(6,3);
    out(0,0) = (2*xi*lmumu*f1) - u*fexp(-3*tau,-3-xi1,L);
    out(1,0) = a*out(0,0)-2*lmumu;
    out(2,0) = a*f3*(2*xi*(e-xi) + (3+xi1)*f0) + f2*(2*xi-e-v*Lf0xi1);
    out(3,0) = out(0,0)*a2 -3*a*lmumu -f1*(e-xi);
    out(4,0) = a*out(2,0) -a*f2*(e + f0xi1-xi) - f1*(1-Lxi1*f0xi1);
    out(5,0) = (a*f3*(2*u-v*(1+v)*f0) + 2*f2*(f0*(L*v-1)*xi12-1))*a + (2-Lxi1)*Lxi1*xi12*fexp(-tau,-1-xi1,L);
    ...
    out(3,2) = out(4,1);
    out(4,2) = out(5,1);
    out(5,2) = ( (e*2*xi1 + f0*(3+xi1)*xi12 -2)*f3*a + (e+(2-L*v)*f0xi1)*3*xi12*f2 )*a2 + (3*a*f1*(2*e+(Lxi1-4)*Lf0xi1) + ( (6-Lxi1)*Lf0xi1 -6*e)*Lxi1 )*xi12*xi1;
...
VectorXd betaInitGev(const Map<MatrixXd>& yi, const Map<MatrixXi>& pFull)
{
  ...
  double yvar(((yi.array() - ymean).matrix().squaredNorm()) /(yi.rows()-1));
  double s(std::sqrt(6*yvar)/Pi); // se(X) when xi == 0
