###############################################################################
########## 1. Run simulations on the poisson model by mgcv (bam) library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    if(class(fit) != "try-error"){
      ...
      outMgcvBam$mu[r] <- mean( (mu-fBam$fitted.values)^2 )
      rm(fBam)
    }
    #save(outMgcvBam, file=paste("../output/out", eval(siz), "PoissMgcvBam.Rdata", sep="")) ## save results for every replicates in case of crash
}
