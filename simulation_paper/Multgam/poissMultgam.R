############################################################################
########## 1. Run simulations on the poisson model by multgam library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    #save(outMultgam, file=paste("../output/out", eval(siz), "PoissMultgam.Rdata", sep="")) ## save results for every replicates in case of crash
}
