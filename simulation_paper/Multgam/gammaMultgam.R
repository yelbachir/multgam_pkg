...
for(r in 1:R){ ## iterate over the replications
    ...
    load(file=paste("../data/data", eval(siz), "Gamma.Rdata", sep=""))
    dat <- Ldat[[r]]
    mu <- Lmu[,r]
    sigma <- Lsigma[,r]
    ...
    if(class(fit) != "try-error"){
      ...
      outMultgam$mu[r] <- mean( (mu - exp(fMultgam$fitted.values[,1]))^2)
      outMultgam$sigma[r] <- mean( (sigma - exp(fMultgam$fitted.values[,2]))^2)
      rm(fMultgam)
    }
    #save(outMultgam, file=paste("../output/out", eval(siz), "GammaMultgam.Rdata", sep="")) ## save results for every replicates in case of crash
}
