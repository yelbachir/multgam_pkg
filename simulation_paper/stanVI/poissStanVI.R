###################################################################################
########## 1. Run simulations on the poisson model by brms (stan VI) library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    #save(outStanVI, file=paste("../output/out", eval(siz), "PoissStanVI", Rstart, ".Rdata", sep="")) ## save results for every replicates in case of crash
}