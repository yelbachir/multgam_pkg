#################################################################################
########## 1. Run simulations on the gauss model by brms (stan VI) library ######
...
## outputs
x <- rep(NA, R)
outStanVI <- list(time=x, mu=x, sigma=x)
rm(x)
...
for(r in 1:R){ ## iterate over the replications
    ...
    #save(outStanVI, file=paste("../output/out", eval(siz), "GaussStanVI", Rstart, ".Rdata", sep="")) ## save results for every replicates in case of crash
}