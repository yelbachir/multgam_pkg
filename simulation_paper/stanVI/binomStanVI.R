####################################################################################
########## 1. Run simulations on the binomial model by brms (stan VI) library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    if(class(fit) != "try-error"){
      ...
      outStanVI$mu[r] <- mean( (mu-binomial()$linkinv(fitted(fStan, dpar="mu", scale="linear")[,1]))^2 )
      rm(fStan)
    }
    #save(outStanVI, file=paste("../output/out", eval(siz), "BinomStanVI", Rstart, ".Rdata", sep="")) ## save results for every replicates in case of crash
}