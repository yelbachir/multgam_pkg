################################################@@@######################################
########## 1. Run simulations on the exponential model by brms (stan MCMC) library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    #save(outStanMCMC, file=paste("../output/out", eval(siz), "ExponStanMCMC", Rstart, ".Rdata", sep="")) ## save results for every replicates in case of crash
}