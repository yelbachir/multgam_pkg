#########################################################################
########## 1. Run simulations on the poisson model by inla library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    #save(outInla, file=paste("../output/out", eval(siz), "PoissInla.Rdata", sep="")) ## save results for every replicates in case of crash
}
