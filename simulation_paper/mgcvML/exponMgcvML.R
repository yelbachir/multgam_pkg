###################################################################################
########## 1. Run simulations on the exponential model by mgcv (gam) library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    if(class(fit) != "try-error"){
      ...
      outMgcvML$mu[r] <- mean( (mu-1/fMgcvML$fitted.values)^2 )
      rm(fMgcvML)
    }
    #save(outMgcvML, file=paste("../output/out", eval(siz), "ExponMgcvML.Rdata", sep="")) ## save results for every replicates in case of crash
}
