###############################################################################
########## 1. Run simulations on the poisson model by mgcv (gam) library ######
...
for(r in 1:R){ ## iterate over the replications
    ...
    if(class(fit) != "try-error"){
      ...
      outMgcvML$mu[r] <- mean( (mu-fMgcvML$fitted.values)^2 )
      rm(fMgcvML)
    }
    #save(outMgcvML, file=paste("../output/out", eval(siz), "PoissMgcvML.Rdata", sep="")) ## save results for every replicates in case of crash
}
